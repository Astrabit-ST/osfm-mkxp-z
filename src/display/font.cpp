...
#define TTF_FONT_TO_FT_FACE(font) (*reinterpret_cast<FT_Face *>(reinterpret_cast<void **>(font) + 1))
...
{
	FT_Face ft_face = TTF_FONT_TO_FT_FACE(font->font);
	FT_ULong len;
	FT_Error err;
                                    ...
                                    int size, float hiresMult, int outline_size)
{
	...
	font = TTF_OpenFontIO(ops, true, 1.0);
	...
	if (font)
	{
		...
		if (font)
		{
			...
			TTF_SetFontHinting(font, (TTF_HintingFlags)p->fontHinting);
		}
	}
