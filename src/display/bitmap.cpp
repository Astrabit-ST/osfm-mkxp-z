...
struct BitmapPrivate
{
    ...
    void allocSurface()
    {
        surface = SDL_CreateSurface(getGLTypes().width, getGLTypes().height, SDL_GetPixelFormatForMasks(format->bits_per_pixel,
                  ...
                  const Vec4 &color)
    {
        if (megaSurface)
        {
            ...
            SDL_FillSurfaceRect(megaSurface, &rect, SDL_MapSurfaceRGBA(megaSurface, r, g, b, a));
        }
        else
        {
            bindFBO();
...
Bitmap::Bitmap(const Bitmap &other, int frame)
{
    ...
    if (other.isMega())
    {
        p->megaSurface = SDL_ConvertSurface(other.p->megaSurface, p->format->format);
    }
    // TODO: Clean me up
...
void Bitmap::childUpdate()
{
    ...
    if (!SDL_GetRectIntersection(&viewportRect, pChild->shared.sceneRect, &viewportRect))
    {
    ...
    if (isWindow)
    {
        ...
        if (!SDL_GetRectIntersection(&viewportRect, &window, &viewportRect))
        {
            pChild->shared.isVisible = false;
            return;
        }
    ...
    if (!isPlane)
    {
        ...
        if (!SDL_HasRectIntersection(&visibleRect, &tmpSourceRect))
        {
            pChild->shared.isVisible = false;
            return;
        }
        if (pChild->shared.angle)
        {
            ...
            if (!SDL_HasRectIntersection(&viewportRect, &tmpSourceRect))
            {
                pChild->shared.isVisible = false;
                return;
            }
                        ...
                        enum BitmapBltMode mode)
{
    ...
    if (p->megaSurface)
    {
        ...
        if (destRect.w < 0 || destRect.h < 0)
        {
            ...
            blitTemp = SDL_CreateSurface(sourceRect.w, sourceRect.h, SDL_GetPixelFormatForMasks(p->format->bits_per_pixel,
        ...
        if(scaleIsOne)
            ...
            SDL_BlitSurfaceScaled(srcSurf, &sourceRect, p->megaSurface, &destRect, SDL_SCALEMODE_NEAREST);
        ...
        {
            SDL_DestroySurface(source.p->surface);
            source.p->surface = 0;
        }
    }
                              ...
                              bool vertical)
{
    ...
    if (p->megaSurface)
    {
        ...
        while (*current < end)
        {
            ...
            Uint32 color = SDL_MapSurfaceRGBA(p->megaSurface, r, g, b, a);
            
            SDL_FillSurfaceRect(p->megaSurface, &destRect, color);
...
void Bitmap::clear()
{
    ...
    {
        SDL_Rect fRect = rect();
        SDL_FillSurfaceRect(p->megaSurface, &fRect, 0);
    }
    else
    {
        p->bindFBO();
...
void Bitmap::replaceRaw(void *pixel_data, int size)
{
    ...
    if (p->megaSurface)
    {
        ...
        if (SDL_GetPixelFormatDetails(p->megaSurface->format)->bits_per_pixel == 32)
...
static void applyShadow(SDL_Surface *&in, const SDL_PixelFormatDetails &fm, const SDL_Color &c, int offset)
{
    SDL_Surface *out = SDL_CreateSurface
    (in->w+offset, in->h+offset, SDL_GetPixelFormatForMasks(fm.bits_per_pixel, fm.Rmask, fm.Gmask, fm.Bmask, fm.Amask));
...
{
    size_t offset = (inRect.x * SDL_GetPixelFormatDetails(txtSrf->format)->bytes_per_pixel) + (inRect.y * txtSrf->pitch);
    uint8_t *txtStart = (uint8_t*)txtSrf->pixels + offset;
    offset = (outRect.x * SDL_GetPixelFormatDetails(outSrf->format)->bytes_per_pixel) + (outRect.y * outSrf->pitch);
    ...
    uint32_t fullTxtPixel = SDL_MapSurfaceRGBA(outSrf, inColor.r, inColor.g, inColor.b, inColor.a);
    uint32_t fullOutPixel = SDL_MapSurfaceRGBA(outSrf, outColor.r, outColor.g, outColor.b, outColor.a);
    ...
    for (int i=0; i < inRect.h; ++i)
    {
        ...
        {
            uint8_t txtA = (*txtPixel >> SDL_GetPixelFormatDetails(txtSrf->format)->Ashift) & 0xFF;
            uint8_t outA = (*outPixel >> SDL_GetPixelFormatDetails(outSrf->format)->Ashift) & 0xFF;
            ...
            if (txtA >= inColor.a)
            {
                ...
                *outPixel = *txtPixel;
                ...
                if (hasShadow)
                {
                    txtR = (*txtPixel >> SDL_GetPixelFormatDetails(txtSrf->format)->Rshift) & 0xFF;
                    txtG = (*txtPixel >> SDL_GetPixelFormatDetails(txtSrf->format)->Gshift) & 0xFF;
                    txtB = (*txtPixel >> SDL_GetPixelFormatDetails(txtSrf->format)->Bshift) & 0xFF;
                }
                ...
                *outPixel = SDL_MapSurfaceRGBA(outSrf, r, g, b, a);
...
void Bitmap::drawText(const IntRect &rect, const char *str, int align)
{
    ...
    if (TTF_MeasureString(sdlFont, str, 0, std::min(width() - rect.x, rect.w) / squeezeLimit, nullptr, &charLimit))
    {
    ...
    if (p->font->isSolid())
        txtSurf = TTF_RenderText_Solid(sdlFont, str, 0, c);
    else
        txtSurf = TTF_RenderText_Blended(sdlFont, str, 0, c);
    ...
    if (scaledOutlineSize)
    {
        ...
        if (p->font->isSolid())
            outline = TTF_RenderText_Solid(sdlOutline, str, 0, co);
        else
            outline = TTF_RenderText_Blended(sdlOutline, str, 0, co);
...
IntRect Bitmap::textSize(const char *str)
{
    ...
    int w, h;
    TTF_GetStringSize(sdlFont, fixed.c_str(), 0, &w, &h);
    
    int ws;
    TTF_GetStringSize(sdlFont, " ", 0, &ws, 0);
    w -= ws;
    ...
    if (p->font->getItalic() && *endPtr == '\0')
        TTF_GetGlyphMetrics(sdlFont, ucs2, 0, 0, 0, 0, &w);
