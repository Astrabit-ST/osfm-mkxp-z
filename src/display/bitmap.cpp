...
struct BitmapPrivate {
  ...
  struct {
    ...
    unsigned int currentFrameI() {
      if (!playing || needsReset)
        return lastFrame;
      int i = currentFrameIRaw();
      return (loop)                         ? fmod(i, frames.size())
             : (i > (int)frames.size() - 1) ? (int)frames.size() - 1
                                            : i;
    }
  ...
  BitmapPrivate(Bitmap *self)
      : self(self), megaSurface(0), selfHires(0), selfLores(0), surface(0),
  ...
  void allocSurface() {
    surface =
        SDL_CreateSurface(gl.width, gl.height,
                          SDL_GetPixelFormatForMasks(
                              format->bits_per_pixel, format->Rmask,
  ...
  void addTaintedArea(const IntRect &rect) {
    ...
    pixman_region_union_rect(&tainted, &tainted, norm.x, norm.y, norm.w,
                             norm.h);
  }
  ...
  void bindTexture(ShaderBase &shader, bool substituteLoresSize = true) {
    ...
    if (animation.enabled) {
      if (selfLores) {
        Debug() << "BUG: High-res BitmapPrivate bindTexture for animations not "
                   "implemented";
      }
    ...
    if (IMG_isGIF(ops)) {
      ...
      do {
        ...
        if (status != GIF_OK && status != GIF_WORKING) {
          ...
          delete gif_data;
          error =
              "Failed to initialize GIF (Error " + std::to_string(status) + ")";
          return false;
        }
      } while (status != GIF_OK);
      ...
      if (status != GIF_OK && status != GIF_WORKING) {
        error = "Failed to decode first GIF frame. (Error " +
...
Bitmap::Bitmap(const char *filename) {
  ...
  // TODO: once C++20 is required, switch to
  // filenameStd.starts_with(hiresPrefix)
  if (shState->config().enableHires &&
      filenameStd.compare(0, hiresPrefix.size(), hiresPrefix) != 0) {
  ...
  try {
    ...
    if (!handler.error.empty()) {
      // Not loaded with SDL, but I want it to be caught with the same exception
      // type
      throw Exception(Exception::SDLError, "Error loading image '%s': %s",
                      filename, handler.error.c_str());
  ...
  if (handler.gif) {
    if (handler.gif->width >= (uint32_t)glState.caps.maxTexSize ||
      ...
      if (hiresBitmap)
        delete hiresBitmap;
      throw new Exception(Exception::MKXPError,
                          "Animation too large (%ix%i, max %ix%i)",
                          handler.gif->width, handler.gif->height,
                          glState.caps.maxTexSize, glState.caps.maxTexSize);
    }
    ...
    if (handler.gif->frame_count == 1) {
      ...
      try {
        texfbo =
            shState->texPool().request(handler.gif->width, handler.gif->height);
      } catch (const Exception &e) {
      ...
      TEX::bind(texfbo.tex);
      TEX::uploadImage(handler.gif->width, handler.gif->height,
                       handler.gif->frame_image, GL_RGBA);
      gif_finalise(handler.gif);
      delete handler.gif;
      delete handler.gif_data;
    ...
    p->animation.fps =
        1 /
        ((float)handler.gif->frames[handler.gif->decoded_frame].frame_delay /
         100);
    ...
    if (fcount > fcount_partial) {
      Debug() << "Non-fatal error reading" << filename << ": Only decoded"
              << fcount_partial << "out of" << fcount << "frames";
    }
    for (int i = 0; i < fcount_partial; i++) {
      if (i > 0) {
        ...
        if (status != GIF_OK && status != GIF_WORKING) {
          ...
          throw Exception(Exception::MKXPError,
                          "Failed to decode GIF frame %i out of %i (Status %i)",
      ...
      try {
        texfbo =
            shState->texPool().request(p->animation.width, p->animation.height);
      } catch (const Exception &e) {
        releaseResources();
      ...
      TEX::bind(texfbo.tex);
      TEX::uploadImage(p->animation.width, p->animation.height,
                       handler.gif->frame_image, GL_RGBA);
...
Bitmap::Bitmap(void *pixeldata, int width, int height) {
  SDL_Surface *surface = SDL_CreateSurface(
      width, height,
      SDL_GetPixelFormatForMasks(p->format->bits_per_pixel, p->format->Rmask,
                                 p->format->Gmask, p->format->Bmask,
                                 p->format->Amask));
  ...
  memcpy(surface->pixels, pixeldata,
         width * height * (p->format->bits_per_pixel / 8));

  if (surface->w > glState.caps.maxTexSize ||
      surface->h > glState.caps.maxTexSize) {
    p = new BitmapPrivate(this);
...
// frame is -2 for "any and all", -1 for "current", anything else for a specific
// frame
Bitmap::Bitmap(const Bitmap &other, int frame) {
  other.guardDisposed();
  other.ensureNonMega();
  if (frame > -2)
    other.ensureAnimated();
...
void Bitmap::initFromSurface(SDL_Surface *imgSurf, Bitmap *hiresBitmap,
                             bool forceMega) {
  p->ensureFormat(imgSurf, SDL_PIXELFORMAT_ABGR8888);

  if (imgSurf->w > glState.caps.maxTexSize ||
...
void Bitmap::blt(int x, int y, const Bitmap &source, const IntRect &rect,
                 int opacity) {
  if (source.isDisposed())
    return;

  stretchBlt(IntRect(x, y, abs(rect.w), abs(rect.h)), source, rect, opacity);
}

static bool shrinkRects(float &sourcePos, float &sourceLen,
                        const int &sBitmapLen, float &destPos, float &destLen,
  ...
  if (!normalize) {
    ...
    // Ensure the source rect has positive dimensions, for blitting from mega
    // surfaces
...
static bool shrinkRects(int &sourcePos, int &sourceLen, const int &sBitmapLen,
  ...
  bool ret = shrinkRects(fSourcePos, fSourceLen, sBitmapLen, fDestPos, fDestLen,
                         dBitmapLen, true);

  if (!ret)
    ret = shrinkRects(fDestPos, fDestLen, dBitmapLen, fSourcePos, fSourceLen,
                      sBitmapLen);
...
void Bitmap::stretchBlt(IntRect destRect, const Bitmap &source,
                        IntRect sourceRect, int opacity, bool smooth) {
  guardDisposed();
  ...
  if (hasHires()) {
    ...
    p->selfHires->stretchBlt(IntRect(destX, destY, destWidth, destHeight),
                             source, sourceRect, opacity);
    return;
  }
  ...
  if (source.hasHires()) {
    ...
    stretchBlt(destRect, *source.getHires(),
               IntRect(sourceX, sourceY, sourceWidth, sourceHeight), opacity);
    return;
  }
  ...
  if (shrinkRects(sourceRect.x, sourceRect.w, source.width(), destRect.x,
                  destRect.w, width()))
    return;
  if (shrinkRects(sourceRect.y, sourceRect.h, source.height(), destRect.y,
                  destRect.h, height()))
    return;
  ...
  if (!srcSurf && opacity == 255 && !touchesTaintedArea) {
    ...
    if (srcSurf) {
      ...
      bool srcSurfTooBig =
          !unpack_subimage && (srcSurf->w > glState.caps.maxTexSize ||
      ...
      if (srcRectTooBig || srcSurfTooBig) {
        ...
        if (srcRectTooBig) {
          ...
          blitTemp = SDL_CreateSurface(
              abs(destRect.w), abs(destRect.h),
              SDL_GetPixelFormatForMasks(p->format->bits_per_pixel,
                                         p->format->Rmask, p->format->Gmask,
                                         p->format->Bmask, p->format->Amask));
          if (!blitTemp)
            throw Exception(Exception::SDLError,
                            "Error creating temporary surface for blitting: %s",
                            SDL_GetError());

          if (smooth) {
            error = SDL_BlitSurfaceScaled(srcSurf, &srcRect, blitTemp, 0,
                                          SDL_SCALEMODE_LINEAR);
            smooth = false;
          } else {
            SDL_Rect tmpRect = {0, 0, blitTemp->w, blitTemp->h};
            error = SDL_BlitSurfaceUncheckedScaled(srcSurf, &srcRect, blitTemp,
                                                   &tmpRect,
          ...
          SDL_Rect tmpRect = {0, 0, blitTemp->w, blitTemp->h};
          error =
              SDL_BlitSurfaceUnchecked(srcSurf, &srcRect, blitTemp, &tmpRect);
        }

        if (error) {
          SDL_DestroySurface(blitTemp);
          throw Exception(Exception::SDLError, "Failed to blit surface: %s",
                          SDL_GetError());
        }
      ...
      if (opacity == 255 && !touchesTaintedArea) {
        if (!subImageFix && sourceRect.w == destRect.w &&
          ...
          TEX::uploadSubImage(destRect.x, destRect.y, destRect.w, destRect.h,
          ...
          if (unpack_subimage) {
            ...
            sourceRect.x = 0;
            sourceRect.y = 0;
            TEX::uploadSubImage(0, 0, sourceRect.w, sourceRect.h,
                                srcSurf->pixels, GL_RGBA);
            GLMeta::subRectImageEnd();
          } else {
            TEX::uploadSubImage(0, 0, srcSurf->w, srcSurf->h, srcSurf->pixels,
                                GL_RGBA);
          }
    ...
    if (opacity < 255 || touchesTaintedArea) {
      ...
      GLMeta::blitRectangle(destRect,
                            IntRect(0, 0, abs(destRect.w), abs(destRect.h)));
      GLMeta::blitEnd();
      ...
      if (srcSurf) {
        ...
        if (unpack_subimage) {
          ...
          TEX::uploadSubImage(0, 0, sourceRect.w, sourceRect.h, srcSurf->pixels,
                              GL_RGBA);
          GLMeta::subRectImageEnd();
        } else {
          TEX::uploadSubImage(0, 0, srcSurf->w, srcSurf->h, srcSurf->pixels,
      ...
      bltSubRect = FloatRect((float)sourceRect.x / sourceWidth,
                             ...
                             ((float)sourceWidth / sourceRect.w) *
                                 ((float)abs(destRect.w) / gpTex.width),
                             ((float)sourceHeight / sourceRect.h) *
                                 ((float)abs(destRect.h) / gpTex.height));
...
void Bitmap::gradientFillRect(const IntRect &rect, const Vec4 &color1,
                              const Vec4 &color2, bool vertical) {
  guardDisposed();
  ...
  if (hasHires()) {
    ...
    p->selfHires->gradientFillRect(IntRect(destX, destY, destWidth, destHeight),
...
void Bitmap::blur() {
  ...
  // TODO: Is there some kind of blur radius that we need to handle for high-res
  // mode?
...
static uint32_t &getPixelAt(SDL_Surface *surf,
                            const SDL_PixelFormatDetails *form, int x, int y) {
...
Color Bitmap::getPixel(int x, int y) const {
  ...
  if (hasHires()) {
    Debug() << "GAME BUG: Game is calling getPixel on low-res Bitmap; you may "
               "want to patch the game to improve graphics quality.";
    ...
    if (w >= 1 && h >= 1) {
      ...
      for (int thisX = xHires;
           thisX < xHires + w && thisX < p->selfHires->width(); thisX++) {
        for (int thisY = yHires;
             thisY < yHires + h && thisY < p->selfHires->height(); thisY++) {
  ...
  return Color(
      (pixel >> p->format->Rshift) & 0xFF, (pixel >> p->format->Gshift) & 0xFF,
...
void Bitmap::setPixel(int x, int y, const Color &color) {
  ...
  if (hasHires()) {
    Debug() << "GAME BUG: Game is calling setPixel on low-res Bitmap; you may "
               "want to patch the game to improve graphics quality.";
  ...
  uint8_t pixel[] = {(uint8_t)clamp<double>(color.red, 0, 255),
  ...
  if (p->surface) {
    ...
    surfPixel =
        SDL_MapSurfaceRGBA(p->surface, pixel[0], pixel[1], pixel[2], pixel[3]);
  }
...
bool Bitmap::getRaw(void *output, int output_size) {
  ...
  if (hasHires()) {
    Debug() << "GAME BUG: Game is calling getRaw on low-res Bitmap; you may "
               "want to patch the game to improve graphics quality.";
  }
...
void Bitmap::replaceRaw(void *pixel_data, int size) {
  ...
  if (hasHires()) {
    Debug() << "GAME BUG: Game is calling replaceRaw on low-res Bitmap; you "
               "may want to patch the game to improve graphics quality.";
  }
  ...
  if (size != w * h * 4)
    throw Exception(
        Exception::MKXPError,
        "Replacement bitmap data is not large enough (given %i bytes, need %i)",
        size, requiredsize);
...
void Bitmap::saveToFile(const char *filename) {
  ...
  if (hasHires()) {
    Debug() << "GAME BUG: Game is calling saveToFile on low-res Bitmap; you "
               "may want to patch the game to improve graphics quality.";
  }
  ...
  if (p->surface || p->megaSurface) {
    ...
    if (!surf)
      throw Exception(Exception::SDLError,
                      "Failed to prepare bitmap for saving: %s",
                      SDL_GetError());
...
void Bitmap::drawText(int x, int y, int width, int height, const char *str,
                      int align) {
...
static void applyShadow(SDL_Surface *&in, const SDL_PixelFormatDetails &fm,
                        const SDL_Color &c) {
  SDL_Surface *out = SDL_CreateSurface(
      in->w + 1, in->h + 1,
      SDL_GetPixelFormatForMasks(fm.bits_per_pixel, fm.Rmask, fm.Gmask,
                                 fm.Bmask, fm.Amask));
  ...
  for (int y = 0; y < in->h + 1; ++y)
    for (int x = 0; x < in->w + 1; ++x) {
      ...
      uint32_t *outP =
          ((uint32_t *)((uint8_t *)out->pixels + y * out->pitch)) + x;
      ...
      if (y > 0 && x > 0)
        shd =
            ((uint32_t *)((uint8_t *)in->pixels + (y - 1) * in->pitch))[x - 1];
...
void Bitmap::drawText(const IntRect &rect, const char *str, int align) {
  ...
  if (hasHires()) {
    ...
    hiresFont.setSize(loresFont.getSize() * p->selfHires->width() / width(),
                      false);
    ...
    p->selfHires->drawText(IntRect(rectX, rectY, rectWidth, rectHeight), str,
                           align);
  ...
  if (p->font->isSolid())
    txtSurf = TTF_RenderText_Solid(font, str, 0, c);
  else
    txtSurf = TTF_RenderText_Blended(font, str, 0, c);
  ...
  if (p->font->getOutline()) {
    ...
    SDL_Rect outRect = {scaledOutlineSize, scaledOutlineSize, txtSurf->w,
                        txtSurf->h};
...
IntRect Bitmap::textSize(const char *str) {
  ...
  // TODO: High-res Bitmap textSize not implemented, but I think it's the same
  // as low-res? Need to double-check this.
...
void Bitmap::setInitFont(Font *value) {
  if (hasHires()) {
    ...
    if (hiresFont && hiresFont != &shState->defaultFont()) {
      // Disable the illegal font size check when creating a high-res font.
      hiresFont->setSize(hiresFont->getSize() * p->selfHires->width() / width(),
                         false);
    }
  }
...
SDL_Surface *Bitmap::megaSurface() const {
  if (hasHires()) {
    if (p->megaSurface) {
      Debug() << "BUG: High-res Bitmap megaSurface not implemented (low-res "
                 "has megaSurface)";
    }
    if (p->selfHires->megaSurface()) {
      Debug() << "BUG: High-res Bitmap megaSurface not implemented (high-res "
                 "has megaSurface)";
    }
  }
...
int Bitmap::addFrame(Bitmap &source, int position) {
  ...
  if (source.height() != height() || source.width() != width())
    throw Exception(
        Exception::MKXPError,
        "Animations with varying dimensions are not supported (%ix%i vs %ix%i)",
  ...
  if (source.surface()) {
    ...
    TEX::uploadImage(source.width(), source.height(), source.surface()->pixels,
                     GL_RGBA);
    SDL_DestroySurface(p->surface);
    p->surface = 0;
  } else {
  ...
  if (position < 0) {
    ...
    p->animation.frames.insert(
        p->animation.frames.begin() +
            clamp(position, 0, (int)p->animation.frames.size()),
        newframe);
    ret = position;
  }
...
void Bitmap::removeFrame(int position) {
  ...
  // Change the animated bitmap back to a normal one if there's only one frame
  // left
