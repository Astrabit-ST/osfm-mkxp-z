...
static void arrayPushBack(std::vector<size_t> &array, size_t size,
                          size_t index) {
...
SoundEmitter::SoundEmitter(const Config &conf)
    : bufferBytes(0), srcCount(conf.SE.sourceCount), alSrcs(srcCount),
      atchBufs(srcCount), srcPrio(srcCount) {
